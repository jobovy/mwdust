 ...
 */
EXPORT float *lambert_getval(char *pFileN,
                             char *pFileS,
                             long nGal,
                             float *pGall,
                             float *pGalb,
         ...
         if (nIndx > 0)
         {
            ...
            { /* INTERPOLATE */
               ...
               for (ii = 0; ii < nIndx; ii++)
               {
                  ...
                  pOutput[pIndx[ii]] =
                      pWeight[0] * pSubimg[pXPix[ii] - pStart[0] + (pYPix[ii] - pStart[1]) * xsize] + pWeight[1] * pSubimg[pXPix[ii] - pStart[0] + 1 + (pYPix[ii] - pStart[1]) * xsize] + pWeight[2] * pSubimg[pXPix[ii] - pStart[0] + (pYPix[ii] - pStart[1] + 1) * xsize] + pWeight[3] * pSubimg[pXPix[ii] - pStart[0] + 1 + (pYPix[ii] - pStart[1] + 1) * xsize];
               }
 ...
 * projection.
 */
void lambert_lb2fpix(float gall, /* Galactic longitude */
 ...
 *
 */
void lambert_lb2pix(float gall, /* Galactic longitude */
                    float galb, /* Galactic latitude */
                    HSIZE nHead,
                    uchar *pHead,
 ...
 * Latitude runs clockwise from X-axis for NGP, counterclockwise for SGP.
 */
void lambert_lb2xy(float gall,  /* Galactic longitude */
                   float galb,  /* Galactic latitude */
